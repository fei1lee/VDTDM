...
def get_optimizer(cfg: MultiViewConfig, model: torch.nn.Module, accelerator: Accelerator) -> torch.optim.Optimizer:
...
def get_scheduler(cfg: MultiViewConfig, optimizer: torch.optim.Optimizer) -> Callable:
...
def resume_from_checkpoint(cfg: MultiViewConfig, model, optimizer=None, scheduler=None, model_ema=None):
...
def print_parameter_quantity(model):
