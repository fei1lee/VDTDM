...
def ensure_point_cloud_has_colors(pointcloud: Pointclouds):
    if pointcloud.features_padded() is None:
        pointcloud = type(pointcloud)(points=pointcloud.points_padded(),
                                      normals=pointcloud.normals_padded(),
                                      features=torch.zeros_like(pointcloud.points_padded()))
    return pointcloud
...
def visualize_pointcloud_batch_pytorch3d(
    ...
    if cameras is None:
        ...
        R, T = look_at_view_transform(dist=10.0, elev=elev, azim=list(range(0, 360, 360 // F)), degrees=True,
                                      device=device)
