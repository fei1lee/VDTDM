...
def main(cfg: MultiViewConfig):
    ...
    train_state: training_utils.TrainState = training_utils.resume_from_checkpoint(cfg, model, optimizer, scheduler,
                                                                                   model_ema)
    data_portion_train = [0, 0.8]
    data_portion_eval = [0.8, 1]
    datasets_train = ShapeNetPointRec(data_portion_train, cfg)
    datasets_test = ShapeNetPointRec(data_portion_eval, cfg)
    ...
    dataloader_test = DataLoader(datasets_test, cfg.dataloader.batch_size, cfg.run.job != 'train',
                                 num_workers=cfg.dataloader.num_workers, pin_memory=True)
    dataloader_vis = DataLoader(datasets_vis, 1, True, num_workers=0)
...
def sample(
    ...
    for batch_idx, batch in enumerate(progress_bar):
        ...
        for sample_idx in range(cfg.run.num_samples):
            ...
            # Sample
            output = model(batch, mode='sample')
            output: Pointclouds
            all_outputs: List[
